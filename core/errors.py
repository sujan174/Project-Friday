...
class ErrorMessageEnhancer:
    """
    Enhances technical error messages into user-friendly explanations.

    Transforms bare errors into helpful messages with:
    ...
    Example:
        ...
        Enhanced: "Issue KAN-123 not found. It may have been deleted or the ID is incorrect.
                   Try: • Check if the issue was recently deleted
    ...
    OPERATION_CONTEXTS = {
        'issue': {
            '404': "Issue {entity} not found",
            '403': "You don't have permission to access issue {entity}",
            '401': "Authentication required to access issue {entity}"
        },
        'pr': {
            '404': "Pull request {entity} not found",
            '403': "You don't have permission to access PR {entity}",
            '401': "Authentication required to access PR {entity}"
        },
        'channel': {
            '404': "Channel {entity} not found",
            '403': "You don't have permission to post in channel {entity}",
            '401': "Authentication required for channel {entity}"
        },
        'repository': {
            '404': "Repository {entity} not found or is private",
            '403': "You don't have permission to access repository {entity}",
            '401': "Authentication required for repository {entity}"
        }
    }
    ...
    def enhance(
        ...
        Args:
            ...
            error_type: Error category (from ErrorCategory)
            context: Additional context (agent, entity, operation, etc.)
        ...
        # Start with classification-based explanation
        classification = ErrorClassifier.classify(error, agent_name)
        ...
        if entity and operation:
            specific_context = ErrorMessageEnhancer._get_specific_context(
                error, operation, entity
            )
            if specific_context:
                parts.append(specific_context)

        # 2. General explanation
        parts.append(classification.explanation)

        # 3. Recovery suggestions (enhanced with context)
        suggestions = ErrorMessageEnhancer._get_contextual_suggestions(
            classification,
            context
        )

        message = parts[0] if parts else classification.explanation
        ...
        related_info = ErrorMessageEnhancer._get_related_info(
    ...
    def _get_specific_context(
        ...
        # Extract error code
        error_code = None
        for code in ['404', '403', '401', '400', '429', '500', '502', '503']:
        ...
        if operation in ErrorMessageEnhancer.OPERATION_CONTEXTS:
            templates = ErrorMessageEnhancer.OPERATION_CONTEXTS[operation]
            if error_code in templates:
                return templates[error_code].format(entity=entity)
    ...
    def _get_contextual_suggestions(
        ...
        """Get suggestions enhanced with context"""
        suggestions = list(classification.suggestions)
        ...
        if classification.category == ErrorCategory.PERMISSION:
            if 'issue' in operation or 'pr' in operation:
                suggestions.append(f"• Check if you're a member of the project/repository")
            if 'slack' in agent_name.lower():
                suggestions.append(f"• Verify the Slack bot has access to the channel")
            if 'github' in agent_name.lower():
                suggestions.append(f"• Ensure your GitHub token has the required scopes")

        elif classification.category == ErrorCategory.RATE_LIMIT:
            if context.get('retry_after'):
                retry_time = context['retry_after']
                suggestions.insert(0, f"• Retry in {retry_time} seconds (automatic)")
            suggestions.append(f"• Consider batching multiple {operation}s together")

        elif classification.category == ErrorCategory.VALIDATION:
            if entity:
                suggestions.append(f"• Verify {entity} exists and is in the correct format")
    ...
    def _get_related_info(
        ...
        if category == ErrorCategory.RATE_LIMIT:
            return "Rate limits help prevent service overload. They typically reset after a few minutes."

        if category == ErrorCategory.PERMISSION:
            agent = context.get('agent', '')
            if agent:
                return f"Check the {agent} agent's authentication credentials and permissions."

        if category == ErrorCategory.CAPABILITY:
            return "Some operations may not be supported by the underlying API. Try an alternative approach."
